  ...
  <div class="app-container">
    ...
    <v-pageTable>
      <el-table
        ...
        row-key="id"
      >
        <el-table-column prop="requestCode" label="订单号" />
        ...
        <el-table-column prop="declarationNumber" label="报关单号" />
        ...
        <el-table-column fixed="right" label="操作" width="420">
          <template slot-scope="scope">
            <el-button v-if="isInbound(scope.row)" type="primary" title="入库" @click="openInbound(scope.row, 'inbound')">
              <i class="iconfont">&#xe644;</i>
            </el-button>
            ...
            <el-button v-if="isReturnGoods(scope.row)" type="primary" title="退货" @click="openInbound(scope.row, 'returnGoods')">
              <i class="iconfont">&#xe604;</i>
            </el-button>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="addAndEdit" :title="title" top="10vh" width="800px" @close="closePop">
      <el-form ref="addForm" :model="addData" :rules="rules" label-width="120px">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="邮箱：" prop="mailbox">
              <el-input v-model="addData.mailbox"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="报关单号：" prop="declarationNumber">
              <el-input v-model="addData.declarationNumber"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="集装箱号：" prop="containerNo">
              <el-input v-model="addData.containerNo"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="代理费用：" prop="agencyCommission">
              <el-input v-model="addData.agencyCommission"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="inAndReturn" :title="title" top="10vh" width="800px" @close="closePop">
      <el-form ref="popForm" :model="popData" label-width="120px">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="订单类型：" prop="itemTypeName">
              <el-input v-model="popData.itemTypeName" disabled/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="制单日期：" prop="documentationDate">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="关联单号：" prop="preparedBy">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="货主编号：" prop="supplierCode">
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="addPro" title="选择产品" top="10vh" width="800px" @close="closeProPop">
      ...
      >
        ...
        <el-table-column prop="expirationTime" label="过期时间" width="150px">
          <template slot-scope="scope">
            <el-date-picker
        ...
        <el-table-column prop="volume" label="体积" width="60px"/>
        <el-table-column prop="weight" label="单重" width="60px"/>
      </el-table>
      <template slot="pagination" slot-scope="props">
        <el-pagination
          :layout="props.layout"
...
export default {
  ...
  methods: {
    ...
    isInbound(row) {
      ...
      return left > 0 && status // 订单状态为(订单确认：2，部分入库：4)，并且剩余数量大于0，可以入库和退货
    },
    // 是否可以退货
    isReturnGoods(row) {
      const left = Number(row.residueQuantity)
      return left > 0 // 剩余数量大于0，可以入库和退货
    },
    // 是否可入账
    ifEntry(row) {
      const entry = Number(row.entryPriceStatus)
      const agencyCommission = Number(row.agencyCommission) // 代理费用
      const status = agencyCommission > 0
    ...
    getTableData(val, flag) {
      ...
      const url = 'kaili-basic/v1/warehousingApplications/queryWarehousingApplication'
      ...
      this.getRequestParams(url, params).then(data => {
        ...
        this.table.body.forEach((e) => {
          ...
          e.children = [].concat(e.warehousingProductList) // 拷贝数组，存放在children字段中，element表格展开用到
    ...
    editHandle(row) {
      ...
      this.submitBody.body = this.addData.warehousingProductList
      this_.$nextTick(() => {
        this_.$refs['addForm'].clearValidate()
      })
    },
    ...
    showHandle(row) {
      ...
      this.submitBody.body = this.popData.warehousingProductList
      this_.$nextTick(() => {
        this_.$refs['popForm'].clearValidate()
      })
    },
    // 打开入库界面
    openInbound(row, string) {
      const this_ = this
    ...
    deleteHandle(row) {
      this.$confirm('确认删除？', '提示', {
        ...
        const url = 'kaili-basic/v1/warehousingApplications/deleteWarehousingApplications'
    ...
    confirmHandle(row) {
      this.$confirm('确认？', '提示', {
        ...
        const url = `kaili-basic/v1/warehousingApplications/${row.applyId}/orderConfirmation`
    ...
    cancelHandle(row) {
      this.$confirm('确认取消？', '提示', {
        ...
        const url = `kaili-basic/v1/warehousingApplications/${row.applyId}/orderCancellation`
        this.putRequest(url, {}).then(() => {
          this.$message.success('操作成功')
          this.getTableData()
        })
    ...
    submitForm() {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          params.warehousingProductList = this_.submitBody.body
          ...
          params.warehousingProductList.forEach(e => {
          ...
          if (this.addAndEdit) {
            if (this_.isAdd) {
              url = 'kaili-basic/v1/warehousingApplications/addWarehousingApplication'
              message = '新增成功'
              index = 0
            } else {
              url = 'kaili-basic/v1/warehousingApplications/updateWarehousingApplication'
              message = '修改成功'
              index = 1
            }
          } else if (this.inAndReturn) { // 入库、退货
            // params.detailList = params.warehousingProductList
            // params.inStoreType = 221
            // params.orderNo = params.requestCode
            if (this_.inbound) {
              url = 'kaili-basic/v1/inStore'
              message = '新增入库成功'
              index = 0
            } else {
              url = 'kaili-basic/v1/warehousingApplications/updateWarehousingApplication'
